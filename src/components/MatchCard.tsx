...
function TeamRow({ name, crest, score, highlight = false, isWinner = false }: {
  ...
  return (
    <motion.div
      className={`flex items-center justify-between gap-2 md:gap-4 p-2 md:p-3 rounded-lg transition-all duration-300 ${
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          ...
          {isWinner && (
            <div className="flex items-center gap-1 mt-0.5 md:mt-1">
              ...
              <span className="text-xs text-success font-medium hidden md:inline">Winner</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <span className={`tabular-nums text-right font-bold text-base md:text-lg ${
...
function MatchCardCmp({
  ...
  return (
    <motion.div
      className="group match-card rounded-xl md:rounded-2xl glass-card border border-border/50 p-4 md:p-6 hover:shadow-2xl hover:border-primary/30 transition-all duration-300 cursor-pointer relative overflow-hidden active:scale-[0.98]"
      whileHover={{
        scale: 1.015,
        y: -2,
        transition: { duration: 0.2 }
    ...
    >
      ...
      <div className="relative flex items-center justify-between gap-2 md:gap-4 mb-4 md:mb-6">
      ...
      <div className="relative space-y-2 md:space-y-3">
        ...
        <div className="flex items-center justify-center py-1 md:py-2">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="h-px bg-border flex-1 w-6 md:w-8" />
            <span className="text-xs font-medium text-muted-foreground bg-muted px-2 py-0.5 md:py-1 rounded-full">
              VS
            </span>
            <div className="h-px bg-border flex-1 w-6 md:w-8" />
          </div>
      ...
      <div className="relative mt-3 md:mt-4 pt-3 md:pt-4 border-t border-border/30 group-hover:border-border/50 transition-colors">
        <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
          <span className="flex items-center gap-1 md:gap-1.5">
            <ClockIcon className="w-3 md:w-3.5 h-3 md:h-3.5 opacity-70" />
            <span className="truncate">{formatKickoffDate(match.utcDate, { weekday: "short", month: "short", day: "numeric" })}</span>
          </span>
